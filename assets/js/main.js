...
const movieElement = (movie) => {
  ...
  contentDiv.onclick = function () {
    // Funktion, die beim Klick auf den Film auf die IMDB-Seite des Films weiterleitet
...
const searchMovie = () => {
  ...
  const searchResult = moviesPlus.filter(
    (movie) => movie[0].toLowerCase().includes(searchInput.toLowerCase()) // Sucheingabe wird mit dem Titel der Filme verglichen. ToLowerCase, damit die Groß- und Kleinschreibung keine Rolle spielt.
  );
  movieContainer.innerHTML = "";
...
document
  ...
  .addEventListener("change", searchGenre);
...
async function fetchMovieDetails(movie) {
  ...
  const response = await fetch(
    `https://www.omdbapi.com/?t=${encodeURIComponent(
      title
    )}&y=${year}&apikey=9443afe5`
  ); // API-Link mit Titel und Jahr des Films als Parameter
...
const addMovie = () => {
  ...
  const movie = [
    ...
    imdbID,
    poster,
  ]; // Eingaben werden in ein Array gespeichert
  ...
  document.getElementById("newMovie").textContent =
